...
sub Run {
    ...
    my $AdditionalFilterName = $Config->{'ViewBy::FilterType'};
    ...
    foreach my $FilterTypeKey ( keys %{$Config->{'ViewBy::FilterAttributes'}} )
    {
        $Counter++;
        ...
        if ( $AdditionalFilterName eq 'Priorities' )
        {
            my $PriorityID = $PriorityObject->PriorityLookup(
                Priority => $Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey},
            );  
        ...
        if ( $AdditionalFilterName eq 'Types' )
        {
            my $TypeID = $TypeObject->TypeLookup( 
                Type => $Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey},
            );  
        ...
        #verify filter value
        if ( $AdditionalFilterName eq 'States' )
        {
            my $StateID = $StateObject->StateLookup( 
                State => $Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey},
            );  
        ...
        $Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey} => {
            Name   => Translatable("$Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey}"),
            Prio   => $Prio,
            Search => {
                StateType  => 'Open',
                $AdditionalFilterName  => [ $Config->{'ViewBy::FilterAttributes'}->{$FilterTypeKey} ],
                OrderBy    => $OrderBy,
                SortBy     => $SortBy,
                UserID     => $Self->{UserID},
                Permission => 'ro',
